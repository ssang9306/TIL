...
나이는 ‘만’으로 계산핚다.)
*/
-- TO DATE에 RRRR형식 지정, 다시 TO_CHAR에서도 RRRR형식 지정 필요
...
*/
SELECT  '2020년 크리스마스는 ' || 
        TO_CHAR(TO_DATE('201225','RRMMDD'),'DAY')|| ' 입니다.'
FROM    DUAL;
/*
...
은 각각 몇 년 몇 월 몇 일을 의미할까?
*/
SELECT  TO_CHAR(TO_DATE('99/10/11','YY/MM/DD'),'YYYY "년" MM "월" DD"일"') AS "99YY",
        TO_CHAR(TO_DATE('49/10/11','YY/MM/DD'),'YYYY "년" MM "월" DD"일"') AS "49YY",
        TO_CHAR(TO_DATE('99/10/11','RR/MM/DD'), 'YYYY "년" MM "월" DD"일"')AS "99RR",
        TO_CHAR(TO_DATE('49/10/11','RR/MM/DD'), 'YYYY "년" MM "월" DD "일"') AS "49RR"
FROM    DUAL;
/*
...
점수는 반올림하여 소수점 이하 핚 자리까지맊 표시한다.
*/
SELECT  ROUND((SUM(POINT)/COUNT(*)),1) 평점
FROM    TB_GRADE
WHERE   STUDENT_NO = 'A517178'
/*
10. 학과별 학생수를 구하여 "학과번호", "학생수(명)" 의 
형태로 헤더를 맊들어 결과값이 출력되도록 하시오.
*/
...
SELECT SUBSTR(TERM_NO,1,4) 년도,
        ROUND((SUM(POINT)/COUNT(*)),1) "년도 별 평점"
FROM TB_GRADE
WHERE   STUDENT_NO = 'A112113'
GROUP BY SUBSTR(TERM_NO,1,4)
ORDER BY 1;

SELECT  TO_CHAR(TO_DATE(SUBSTR(TERM_NO,1,4),'RRRR'),'RRRR') 년도,
        ROUND((SUM(POINT)/2),1) "년도 별 평점"
FROM    TB_GRADE
WHERE   STUDENT_NO = 'A112113'
...
총 평점을 구하는 SQL 문을 작성하시오. (단, 평점은 소수점 1자리까지맊 반올림하여 표시한다.)
*/
SELECT  SUBSTR(TERM_NO,1,4) 년도,
...
ORDER BY 1;
/*
4. 현재 법학과 교수 중 가장 나이가 맋은 사람부터 이름을 확인할 수 있는 SQL 문장을 작성하시오. 
...
*/
-- 소수점 0을 표시하려면 TO_CHAR, '99.999'로 9를 원하는 자리만큼 적용
SELECT  STUDENT_NO,
...
SELECT  STUDENT_NO 학번,
        STUDENT_NAME "학생 이름",
        ROUND(SUM(G.POINT)/COUNT(*),1) "전체 평점"
FROM    TB_STUDENT S
JOIN    TB_DEPARTMENT D USING(DEPARTMENT_NO)
JOIN    TB_GRADE G USING(STUDENT_NO)
...
지도 교수에게 내용을 전달하기 위핚 학과 이름, 학생 이름과 지도 교수 이름이 필요하다. 
이때 사용할 SQL 문을 작성하시오. 
...
SELECT      S.STUDENT_NAME 학생이름,
            (CASE WHEN P.PROFESSOR_NAME IS NULL THEN '지도교수 미지정'
...
/*
16. 환경조경학과 전공과목들의 
과목 별 평점을 파악할 수 있는 SQL 문을 작성하시오.*/
SELECT  C.CLASS_NO,
        C.CLASS_NAME,
        G.POINTS
FROM    TB_CLASS C
...
SELECT  D.DEPARTMENT_NAME "계열 학과 명",
        ROUND( (SUM(G.GRADE) / COUNT(*)), 1) 전공평점
FROM    TB_DEPARTMENT D
